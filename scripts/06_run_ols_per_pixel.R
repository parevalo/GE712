...
get_stat <- function(x, args.list){
  ...
  res <- c(intercept          = m1$coefficients[1], # Why the equation is different from the otehrs lm(x[1:13] ~ x[14:26]) ???
           ...
           beta_temp2_opt     = m3$coefficients[3],
           signif_precip_opt  = sm2$coefficients[2,4],
           signif_precip2_opt = sm2$coefficients[3,4],
           signif_temp_opt    = sm3$coefficients[2,4],
           signif_temp2_opt   = sm3$coefficients[3,4]
  )
...
save_maps = function(w, x, y, z,index, location, fname, ...){
  ...
  plot(w[[index]], col=brewer.pal(6, "RdBu"), legend.only=T, legend.width=1, legend.shrink=1, side=4)
...
# Plot optimum temp
save_maps(djf_optimum_zone1, mam_optimum_zone1, jja_optimum_zone1, son_optimum_zone1, 1, sa, "filtered_optimum_temp_005_zone1.png")
save_maps(djf_optimum_zone1, mam_optimum_zone1, jja_optimum_zone1, son_optimum_zone1, 2, sa, "filtered_optimum_precip_005_zone1.png")

save_maps(djf_optimum_zone2, mam_optimum_zone2, jja_optimum_zone2, son_optimum_zone2, 1, sa, "filtered_optimum_temp_005_zone2.png")
save_maps(djf_optimum_zone2, mam_optimum_zone2, jja_optimum_zone2, son_optimum_zone2, 2, sa, "filtered_optimum_precip_005_zone2.png")

save_maps(djf_optimum_zone3, mam_optimum_zone3, jja_optimum_zone3, son_optimum_zone3, 1, sa, "filtered_optimum_temp_005_zone3.png")
save_maps(djf_optimum_zone3, mam_optimum_zone3, jja_optimum_zone3, son_optimum_zone3, 2, sa, "filtered_optimum_precip_005_zone3.png")
